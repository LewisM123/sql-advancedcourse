...
SELECT   ps.Hospital,
         ...
         ps.Tariff,
         (SELECT AVG(Tariff)
          FROM   PatientStay) AS AverageTariff
FROM     PatientStay AS ps
WHERE    ps.Tariff > (SELECT AVG(Tariff)
                      FROM   PatientStay)
ORDER BY ps.Tariff;
...
SELECT   ps.Hospital,
         ...
         ps.Tariff,
         (SELECT AVG(Tariff)
          FROM   PatientStay) AS OverallAverageTariff,
         (SELECT AVG(Tariff)
          FROM   PatientStay
          WHERE  Hospital = ps.Hospital) AS HospitalAverageTariff
FROM     PatientStay AS ps
WHERE    ps.Tariff > (SELECT AVG(Tariff)
                      FROM   PatientStay
                      WHERE  PatientStay.Hospital = ps.Hospital)
ORDER BY ps.Hospital, ps.Tariff;
...
SELECT   ps.Hospital,
         ...
         (SELECT SUM(Tariff)
          FROM   PatientStay
          WHERE  Hospital = ps.Hospital) AS TotalHospitalTariff,
         100.0 * ps.Tariff / (SELECT SUM(Tariff)
                              FROM   PatientStay
                              WHERE  Hospital = ps.Hospital) AS HospitalPercentTariff
FROM     PatientStay AS ps
ORDER BY ps.Hospital, ps.PatientId;
...
SELECT   ps.PatientId,
         ...
         (SELECT MIN(PatientId)
          FROM   PatientStay
          WHERE  PatientId > ps.PatientId) - (SELECT MAX(PatientId)
                                              FROM   PatientStay
                                              WHERE  PatientId < ps.PatientId) AS Gap
FROM     PatientStay AS ps
ORDER BY Gap DESC;

SELECT   abc.PatientId,
         abc.PreviouslyBookedPatientId,
         abc.NextBookedPatientId,
         abc.NextBookedPatientId - abc.PreviouslyBookedPatientId AS Gap
...
SELECT   ps.PatientId,
         ...
         (SELECT SUM(Tariff)
          FROM   PatientStay
          WHERE  PatientId <= ps.PatientId) AS RunningTotalTariff
FROM     PatientStay AS ps
