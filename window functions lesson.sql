...
*/
SELECT   DATENAME(MONTH, ps.AdmittedDate) AS AdmittedMonth,
         COUNT(*) AS [Number Of Patients]
FROM     dbo.PatientStay AS ps
GROUP BY DATENAME(MONTH, ps.AdmittedDate);
...
SELECT   ps.PatientId,
         ...
         (SELECT COUNT(*)
          FROM   PatientStay AS ps2) AS TC
FROM     -- create a window over the whole table
         PatientStay AS ps
 ...
 */
WITH   cte
AS     (SELECT ps.AdmittedDate,
               DATENAME(MONTH, ps.AdmittedDate) AS MonthAdmitted,
               ps.Tariff
        FROM   PatientStay AS ps
        WHERE  ps.Hospital = 'Oxleas'
               AND ps.Ward = 'Dermatology')
SELECT cte.MonthAdmitted,
       cte.AdmittedDate,
       cte.Tariff,
...
*/
WITH     cte
AS       (SELECT   ps.Hospital,
                   ps.AdmittedDate,
                   SUM(ps.Tariff) AS TotalTariff
          FROM     PatientStay AS ps
          GROUP BY ps.Hospital, ps.AdmittedDate)
SELECT   cte.Hospital,
         cte.AdmittedDate,
         cte.TotalTariff,
...
FROM     cte
ORDER BY cte.Hospital, cte.AdmittedDate;
...
SELECT   rp.Hospital,
         ...
         rp.Tariff,
         rp.PatientRank
FROM     RankedPatient AS rp
WHERE    rp.PatientRank <= 2
ORDER BY rp.Hospital, rp.Tariff DESC;
