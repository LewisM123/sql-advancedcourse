...
SELECT   ps.PatientId,
         ps.Hospital,
         ps.Ward,
         CASE WHEN ps.Ward LIKE '%Surgery' THEN 'Surgical' WHEN ps.Ward IN ('Accident', 'Emergency', 'Emergency Surgery') THEN 'A&E' WHEN ps.Tariff > 8 THEN 'ICU' ELSE 'General' END AS WardType
FROM     dbo.PatientStay AS ps
ORDER BY WardType;
